call sys.drop('auth.password_reset_code');
...
begin
    ...
    if sys.validate_email(_email) is false then
        call auth.log('password_reset_code', false, _email, format('Invalid email %s in reset attempt.', _email), _analytics_data);
    ...
    if _user_id is null then
        call auth.log('password_reset_code', false, _email, format('User with email %s not found in reset attempt.', _email), _analytics_data);
        -- return success, we don't want to leak if the email exists
        return sys.generate_rnd_code(6);
    end if;

    _reset_code = (
        select sys.generate_rnd_code(_code_len)
    );
    
    _expires_at = now() + _link_expires_in;

    insert into auth_email_resets (
        expires_at,
        email, 
        code, 
        user_id, 
...
call sys.annotate('auth.password_reset_code', 
    'HTTP POST', 
    'Anonymous'
);