call sys.drop('auth.validate_password');
...
begin 
    if _password is null or _password = '' then
        return auth.create_register_response(
            2, 'Password is required.', _email, _analytics_data, _log_type
        );
    end if;

    if _password <> _repeat then
        return auth.create_register_response(
            3, 'Passwords do not match.', _email, _analytics_data, _log_type
        );
    end if;

    if length(_password) < _min_password_length then
        return auth.create_register_response(
            4, format('Password must be at least %s characters.', _min_password_length), _email, _analytics_data, _log_type
        );
    end if;
    ...
    if coalesce(_required_uppercase, 0) > 0 and (
        ...
        return auth.create_register_response(
            6, format('Password must contain at least %s uppercase letter.', _required_uppercase), _email, _analytics_data, _log_type
        );
    end if;
    ...
    if coalesce(_required_lowercase, 0) > 0 and (
        ...
        return auth.create_register_response(
            7, format('Password must contain at least %s lowercase letter.', _required_lowercase), _email, _analytics_data, _log_type
        );
    end if;
    ...
    if coalesce(_required_number, 0) > 0 and (
        ...
        return auth.create_register_response(
            8, format('Password must contain at least %s number.', _required_number), _email, _analytics_data, _log_type
        );
    end if;
    ...
    if coalesce(_required_special, 0) > 0 and (
        ...
        return auth.create_register_response(
            9, format('Password must contain at least %s special character.', _required_special), _email, _analytics_data, _log_type
        );
    end if;
