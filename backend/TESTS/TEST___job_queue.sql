...
begin
    ...
    assert _record.status = 'P', 'Expected status to be P, got ' || _record.status;
    ...
    select * into _record from job_queue where job_queue_id = _job_id;
    assert _record.status = 'R', 'Expected status to be R after dequeue, got ' || _record.status;
    ...
    select * into _record from job_queue where job_queue_id = _job_id;
    assert _record.status = 'C', 'Expected status to be C after completion, got ' || _record.status;
    ...
    assert _record.status = 'P', 'Expected status to be P after failure with retry, got ' || _record.status;
    ...
    assert _record.status = 'F', 'Expected status to be F after max attempts, got ' || _record.status;
    ...
    select * into _record from job_queue where job_queue_id = _job_id;
    assert _record.status = 'R', 'Expected status to still be R with wrong worker ID';
